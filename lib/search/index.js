...
class Search {
	...
	records (searchTerms, cb) {
		searchTerms = searchTerms
		function limiterCb (err, remainingRequests) {
			...
			if (err) {
				...
				amClient.itemSearch(options, itemSearchCb)
			}
		}
		function itemSearchCb (error, results, response) {
			if (error) {
				cb(error)
			} else {
		...
		limiter.removeTokens(1, limiterCb)
	}
}
